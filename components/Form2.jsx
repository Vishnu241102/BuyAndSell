...
export default function Form2() {
  ...
  const [isNego, setIsNego] = useState('');
  ...
  const handleOPriceChange = (event) => {
    setOprice(event.target.value);
  };

  const handleSPriceChange = (event) => {
    setSprice(event.target.value);
  };

  const handleIsNegoChange = (event) => {
    setIsNego(event.target.value);
  };
  ...
  const handleSubmit =  async (event) => {
    event.preventDefault();
      if (!product || !file || !oPrice || !sPrice ||  !contact || !isNego) {
        setWarnToast(true);
      ...
      formData.append('isNego', isNego);
      formData.append('category',category)
      formData.append('contact',contact)
      formData.append('email', session.user.email);
  
      try {
        const response = await fetch('/api/upload/form2Handle', {
          method: 'POST',
          body: formData,
        });
        if (!response.ok) {
      ...
      <div className="max-w-md mx-auto my-8 px-2">
        ...
        (
          <div>
            ...
            <div className="mb-4">
              <label htmlFor="title" className="block text-gray-700 font-bold mb-2">
                  Is Negotiable <span className='text-red-500'>*</span>
              </label>
              <select
                id="isNegotiable"
                ...
                value={isNego}
                onChange={handleIsNegoChange}
              >
                <option value="">Select an option</option>
                <option className='text-green-500 font-bold' value="yes">Yes</option>
                <option className='text-red-500 font-bold' value="no">No</option>
              </select>
            </div>
            <div className="mb-4">
              <label htmlFor="title" className="block text-gray-700 font-bold mb-2">
              ...
              >
                ...
                <option className='text-md font-bold' value="home">Room Items</option>
