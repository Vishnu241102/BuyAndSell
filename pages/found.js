...
export default function buy()
{
    ...
    useEffect(() => {
        ...
        async function fetchData() {
            try
            {
                const res = await axios.get(`http://localhost:4000/getFoundType?search=${inputValue}&category=${categoryFilter}&sort=${sortFilter}`);
    ...
    return(
        ...
        <div className={
            ...
            (
                <div className=' text-[#333333] text-[16px] font-sans flex flex-col font-medium rounded-lg'> 
                    ...
                    <button onClick={()=>signIn()} className='flex flex-row rounded-lg shadow-lg m-2 p-3 border-2 border-slate-200'>
                        <div>
        ...
        <div className='lg:mx-16 text-md font-bold bg-[#11d7ac] w-auto h-auto rounded-lg px-2 py-2 shadow-lg text-center text-white '>
            Lost? Search Here..  <span className='text-xs font-normal text-black'>All Found Items Inside the Campus</span>
        </div>
